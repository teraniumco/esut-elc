    ...
    <div class="flex gap-10">
        ...
        <article class="flex-1 min-w-0">
            ...
            <nav class="flex items-center gap-2 text-xs text-gray-400 mb-6">
                <a href="{{ route('faq.index') }}" class="hover:text-crimson transition-colors">Legal Resources</a>
                <span>›</span>
                <a href="{{ route('faq.category', $category->slug) }}" class="hover:text-crimson transition-colors">{{ $category->name }}</a>
                <span>›</span>
            ...
            {{-- Article header --}}
            <header class="mb-8">
                <span class="inline-flex items-center gap-1.5 bg-crimson/5 text-crimson text-xs font-semibold px-3 py-1 rounded-full mb-4">
                    {{ $category->icon ?? '⚖️' }} {{ $category->name }}
                </span>
                <h1 class="font-serif text-2xl sm:text-3xl font-bold text-crimson leading-snug mb-3">{{ $article->title }}</h1>
            ...
            {{-- Need more help --}}
            <div class="mt-10 bg-crimson/5 border border-crimson/10 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <p class="font-semibold text-crimson">Still need help with your specific situation?</p>
                    <p class="text-sm text-gray-500 mt-0.5">Submit a free enquiry and a student advisor will respond to your case personally.</p>
                </div>
                <a href="{{ route('enquiry.create') }}" class="flex-shrink-0 bg-crimson text-white text-sm font-semibold px-5 py-2.5 rounded-xl hover:bg-gold hover:text-crimson transition-colors">
                    Get Free Help
                </a>
            </div>
        </article>
        ...
        <aside class="hidden lg:block w-64 flex-shrink-0">
            <div class="sticky top-24 space-y-6">
                ...
                <div class="bg-gray-50 rounded-2xl p-5">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('faq.show', [$category->slug, $rel->slug]) }}"
                           class="group block text-sm text-gray-700 hover:text-crimson transition-colors leading-snug">
                            {{ $rel->title }}
                        </a>
                        @endforeach
                    </div>
                ...
                <div class="bg-crimson rounded-2xl p-5 text-center">
                    ...
                    <a href="{{ route('enquiry.create') }}" class="block w-full bg-gold text-crimson text-sm font-bold py-2.5 rounded-xl hover:bg-gold/90 transition-colors">
                        Submit Enquiry
                    </a>
                </div>
            </div>
